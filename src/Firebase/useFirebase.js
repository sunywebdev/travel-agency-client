...
const useFirebase = () => {
	...
	const saveUserToDb = (email, displayName, navigate, location) => {
		...
		axios
			.post(`https://pure-forest-30659.herokuapp.com/users`, save)
	...
	const saveOrReplaceUserToDb = (
		...
		axios
			.put(`https://pure-forest-30659.herokuapp.com/users`, save)
			.then(function (response) {
				Swal.fire({
					icon: "success",
					title: "Login Successfull",
	...
	useEffect(() => {
		fetch(`https://pure-forest-30659.herokuapp.com/users/${user?.email}`)
			.then((res) => res.json())
